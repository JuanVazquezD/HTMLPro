...
const App: React.FC = () => {
    ...
    elements: [
      {
        ...
        type: 'title',
        headingLevel: 'h1',
        content: 'Constructor Visual HTML',
        styles: { fontSize: '42px', textAlign: 'center', margin: '0', fontWeight: '800', color: '#1e293b' }
      },
      {
        id: '2',
        type: 'paragraph',
        content: 'Arrastra elementos y personalízalos. Las imágenes ahora aceptan URLs externas en el panel derecho.',
        styles: { textAlign: 'center', fontSize: '16px', color: '#64748b', margin: '0' }
      }
    ],
    selectedId: null,
    viewMode: 'web',
  ...
  const createNewElement = (type: ElementType): CanvasElement => {
    ...
    let defaultContent: any = 'Texto nuevo';
    if (type === 'image') defaultContent = 'https://images.unsplash.com/photo-1579546929518-9e396f3cc809?q=80&w=600&auto=format&fit=crop';
    if (type === 'columns') defaultContent = '';

    return {
      id, type, content: defaultContent,
      styles: { margin: '0', padding: '10px', textAlign: 'left' },
  ...
  const deleteElementRecursive = (id: string) => {
    const del = (list: CanvasElement[]): CanvasElement[] => {
      return list
        .filter(e => e.id !== id)
        .map(e => {
          if (e.columnData) {
            return {
              ...e,
              columnData: e.columnData.map(c => ({
                ...c,
                elements: del(c.elements)
              }))
            };
          }
          return e;
  ...
  const updateElement = (id: string, updates: Partial<CanvasElement>) => {
    const updateNested = (elements: CanvasElement[]): CanvasElement[] => {
      return elements.map(el => {
        if (el.id === id) return { ...el, ...updates };
        if (el.columnData) {
          return { ...el, columnData: el.columnData.map(col => ({ ...col, elements: updateNested(col.elements) })) };
        }
        return el;
      });
    };
    const next = updateNested(state.elements);
  ...
  const updateStyles = (id: string, styleUpdates: any) => {
    const updateRecursive = (elements: CanvasElement[]): CanvasElement[] => {
      return elements.map(el => {
        ...
        if (el.columnData) {
          return { ...el, columnData: el.columnData.map(col => ({ ...col, elements: updateRecursive(col.elements) })) };
        }
        return el;
      });
    };
    const next = updateRecursive(state.elements);
    setState(prev => ({ ...prev, elements: next }));
    saveToHistory(next);
  };
  ...
  const moveElement = (id: string, direction: 'up' | 'down' | 'top' | 'bottom') => {
    const moveInList = (list: CanvasElement[]): CanvasElement[] => {
      ...
      return list.map(el => el.columnData ? {
        ...el, columnData: el.columnData.map(col => ({ ...col, elements: moveInList(col.elements) }))
      } : el);
    };
    const next = moveInList(state.elements);
    setState(prev => ({ ...prev, elements: next }));
    saveToHistory(next);
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (oldIdx !== -1 && newIdx !== -1) {
        const next = arrayMove(state.elements, oldIdx, newIdx);
        setState(prev => ({ ...prev, elements: next }));
        saveToHistory(next);
      }
    }
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-slate-50 overflow-hidden">
      <TopNav 
        viewMode={state.viewMode} setViewMode={m => setState(s => ({...s, viewMode: m}))} 
        onUndo={undo} onRedo={redo} canUndo={historyIndex > 0} canRedo={historyIndex < history.length - 1} 
        zoom={state.zoom} setZoom={z => setState(s => ({...s, zoom: z}))} elements={state.elements} 
      />
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-12 flex flex-col items-center">
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <div 
              ...
              className={`canvas-container bg-white min-h-[11in] transition-all duration-300 relative mx-auto ${
                state.viewMode === 'mobile' ? 'w-[375px]' : state.viewMode === 'email' ? 'w-[600px]' : 'w-full max-w-[800px]'
              }`}
            >
              <SortableContext items={state.elements.map(e => e.id)} strategy={verticalListSortingStrategy}>
                <Canvas 
                  elements={state.elements} selectedId={state.selectedId} 
                  setSelectedId={id => setState(s => ({...s, selectedId: id}))} 
                  updateElement={updateElement} updateStyles={updateStyles}
                  addElementToColumn={(colId: string, type: ElementType) => {
                    const newEl = createNewElement(type);
                    const updateCols = (elements: CanvasElement[]): CanvasElement[] => elements.map(el => el.columnData ? { ...el, columnData: el.columnData.map(col => col.id === colId ? { ...col, elements: [...col.elements, newEl] } : { ...col, elements: updateCols(col.elements) }) } : el);
                    const next = updateCols(state.elements);
                    setState(p => ({...p, elements: next, selectedId: newEl.id}));
                    saveToHistory(next);
                  }} 
        ...
        <SidebarRight 
          ...
          globalGap={state.globalGap} setGlobalGap={g => setState(s => ({...s, globalGap: g}))}
          updateElement={updateElement} updateStyles={updateStyles} 
          deleteElement={deleteElementRecursive} 
          moveElement={moveElement} 
        />
      </div>
      <BottomBar selectedId={state.selectedId} updateStyles={updateStyles} savedSvgs={state.savedSvgs} onSaveSvg={s => setState(p => ({...p, savedSvgs: [...p.savedSvgs, s]}))} onInsertSvg={() => addElement('svg')} />
    </div>
  );
};
